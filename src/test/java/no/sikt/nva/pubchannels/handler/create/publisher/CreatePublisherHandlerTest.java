...
class CreatePublisherHandlerTest {
    ...
    private static InputStream constructRequest(CreatePublisherRequest body) throws JsonProcessingException {
        return new HandlerRequestBuilder<CreatePublisherRequest>(dtoObjectMapper)
                .withBody(body)
                .build();
    }

    private static void stubResponse(String expectedPid, int statusCode, DataportenCreatePublisherRequest request)
            throws JsonProcessingException {
        var body = new DataportenCreatePublisherResponse(expectedPid);
        stubFor(
                post("/createpublisher/createpid")
