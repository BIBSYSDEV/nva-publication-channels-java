...
public abstract class CreateHandlerTest {
    ...
    protected static void stubResponse(int statusCode, String url, String body, String request) {
        stubFor(
            post(url)
                ...
                .willReturn(
                    aResponse().withStatus(statusCode).withBody(body)
    ...
    protected static void stubGetResponse(int statusCode, String url, String body) {
        stubFor(get(url).withHeader(HttpHeaders.ACCEPT, WireMock.equalTo(HttpHeaders.CONTENT_TYPE_APPLICATION_JSON))
                        .willReturn(aResponse().withHeader(HttpHeaders.CONTENT_TYPE,
    ...
    void commonBeforeEach(WireMockRuntimeInfo runtimeInfo) {
        ...
        publicationChannelClient = new ChannelRegistryClient(httpClient, dataportenBaseUri, dataportenAuthSource);
    }
}
