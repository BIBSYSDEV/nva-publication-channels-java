...
public abstract class SearchByQueryHandlerTest {
  ...
  void shouldReturnContentNegotiatedContentWhenRequested(MediaType mediaType)
    ...
    assertThat(
        actualSearchResult.getHits(), containsInAnyOrder(expectedSearchResult.getHits().toArray()));
  ...
  void shouldReturnResultWithSuccessWhenQueryIsName()
    ...
    assertThat(response.getStatusCode(), is(equalTo(HttpURLConnection.HTTP_OK)));
    assertThat(
        actualSearchResult.getTotalHits(), is(equalTo(expectedSearchResult.getHits().size())));
    assertThat(
        actualSearchResult.getHits(), containsInAnyOrder(expectedSearchResult.getHits().toArray()));
  }
  ...
  void shouldReturnResultWithIdMatchingRequest() throws IOException {
    ...
    var testChannel = new TestChannel(year, pid, type).withPrintIssn(issn);
    var expectedId =
        UriWrapper.fromUri(selfBaseUri)
  ...
  void shouldReturnIdWithOffsetMatchingRequest(int offset) throws IOException {
    ...
    assertThat(
        actualSearchResult.getId().toString(), containsStringIgnoringCase("offset=" + offset));
  }
