...
public abstract class CreateHandler<I, O> extends ApiGatewayHandler<I, O> {
  ...
  protected CreateHandler(Class<I> iclass, Environment environment) {
    ...
    var pubChannelBaseUri =
        URI.create(environment.readEnv(ENV_DATAPORTEN_PUBLICATION_CHANNEL_BASE_URI));
    var authClient =
        new DataportenAuthClient(
            HttpClient.newBuilder().build(), authBaseUri, clientId, clientSecret);
    this.publicationChannelClient =
        new ChannelRegistryClient(HttpClient.newBuilder().build(), pubChannelBaseUri, authClient);
  }
